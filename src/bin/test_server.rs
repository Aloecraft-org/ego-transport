...
async fn run_native_server(addr: &str) {
    ...
    ServerBuilder::new(listener)
        ...
        .run(|mut transport| async move {
            ...
            log::info!("[Server] Handler finished (processed {} messages)", msg_count);
        })
        .await
        .ok();
}
...
async fn run_wasi() {
    ...
    let listener = TcpStreamWasi::bind(addr)
        .await
        .expect("Failed to bind");
    ...
    ServerBuilder::new(listener)
        ...
        .run(|mut transport| async move {
            ...
            log::info!("[WASI Server] Handler finished (processed {} messages)", msg_count);
